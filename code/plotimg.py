...
imgs = glob('files/100StepsPNG/*.png')
carts = glob('files/18650Cross/cartoon*.png')

trfiles = sorted(glob(sdir+'Demo5_*TR.json'))
pefiles = sorted(glob(sdir+'Demo5_*PE.json'))
...
# filename = 'files/new.mp4'
# anim = animation.FuncAnimation(fig, animate, frames=101,blit=True)
# anim.save(filename, fps=15, extra_args=['-vcodec', 'libx264'])    # cur 